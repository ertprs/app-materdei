...
exports.SequelizeSubscribersRepository = void 0;
const sequelize_1 = require("sequelize");
class SequelizeSubscribersRepository {
    constructor() {
        try {
            this.sequelize = new sequelize_1.Sequelize(`${process.env.MYSQL_DATABASE}`, `${process.env.MYSQL_USER}`, `${process.env.MYSQL_PASS}`, {
    ...
    async findByEmailAndType(email, type) {
        return await this.sequelize.query('SELECT * FROM subscribers WHERE email = :email AND type = :type', {
            replacements: {
                email: email,
                type: type
            },
...
exports.SequelizeSubscribersRepository = SequelizeSubscribersRepository;
